...
These are two Hadoop XML configuration files, which must be put into
...
needs to be set during the tests.

### `auth-keys.xml`

This file triggers the testing of the Aliyun OSS classes. Without this file,
...
The contents of the bucket will be cleaned during the test process, so
